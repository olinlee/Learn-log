...
35. 你对首屏渲染做过哪些优化
    ...
    优化分类：
    	加载时优化 1 - 10
    	运行时优化 11 -
    1. 减少http请求：多文件请求合并为单文件，因为除了真正下载数据的时间，一次连接包含了太多其它的时间消耗。
    2. 使用http2：解析速度快，多路复用，首部压缩，服务器推送，优先级，流量控制，
    3. 使用服务端渲染
    4. 静态资源使用 CDN
    5. 将 CSS 放在文件头部，JavaScript 文件放在底部
    6. 使用字体图标 iconfont 代替图片图标
    7. 善用缓存，不重复加载相同的资源
    8. 压缩文件：js、css、图片的压缩（webpack），开启gzip
    9. 图片优化：
    	图片延迟加载（懒加载），
    	响应式图片（选择加载），
    	调整图片大小（预览图，需要时才下载原图），
    	尽可能利用 CSS3 效果代替图片，
    	使用 webp 格式的图片
    10. 通过 webpack 按需加载代码（import（）），提取第三库代码（splitChunk），减少 ES6 转为 ES5 的冗余代码
    11. 减少重绘重排：className 替换 style 修改 ； 使用 DocumentFragement
    12. 使用事件委托
    13. js中 if 与 switch 性能上差距不大， 但是可读性上差距很大，合理选择
    14. 使用 Web Workers 处理计算密集型操作
    15. 使用位运算：
    	取模 %
    	取整 ~~
    16. 原生方法能做到的事，尽量交给它们（c/c++实现的）
    17. transforms 和 opacity 这两个属性更改不会触发重排与重绘
    18. flex 布局性能较高，可多使用，但兼容性稍差
    ```
