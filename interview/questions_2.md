...
7. 负责过团队内部的哪些部分的技术支撑
   ...
   ```
   前端目前救我一个人，所有支撑都在我身上，团队其它成员，前端涉足都比较少。
   ```

30. 搭建一些基础组件的时候, 你会考虑哪些东西

    ```
    1. 标准性：建立组件的统一标准，如分类，文件组织结构，命名，文档书写；
    2. 独立性：描述了组件的细粒度，遵循单一职责原则，保持组件的纯粹性属性配置等API对外开放，组件内部状态对外封闭，尽可能的少与业务耦合；
    3. 粒度小：尽量对组件进行细粒度的划分，实现高可复用；
    	
    ```

31. 函数式编程你是怎么理解的, 高阶函数有用到过吗

    ```
    1. 把函数作为参数传递给另一个函数，也就是所谓的高阶函数
    2. 可以返回一个函数，这样就可以实现闭包或者惰性计算
    3. 函数式编程最大的好处是引用透明，即函数运行的结果只依赖于输入的参数，而不依赖于外部状态，因此，我们常常说函数式编程没有副作用。
    ```

32. 函数柯里化有什么作用

    ```
    1. 柯里化的产生来源于，js中函数的身份比较特殊（当然好些语言都是可以），是一等公民。
    2. 函数柯里化就是一个逐次消元的过程。当把函数的元全消掉，就得到了值。 值就是零元函数。
    3. 作用就是大大提高函数重用的可能，一个3参函数，部分应用可产生6个函。数
    ```

33. 详细的介绍一下你的开源项目

34. ~~我看你还用改nuxt开发项目~~, 除此之外你还了解哪些vue的ssr方案
...
36. ~~vue中的mixins用过吗, 你还有其他的替代方案吗~~

37. ~~vuex有用过吗, vuex是如何实现响应式的~~

38. ~~vue的响应式是如何实现的~~

39. ~~vue的router你用的那种~~, 你觉得hash相较于history会好吗
